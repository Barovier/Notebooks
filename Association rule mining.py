...
###
latin_text2 = get_normalized_words(latin_text)
words = make_itemsets(latin_text2)
latin_rules = find_assoc_rules(words, 0.75)
...
###
english_text2 = get_normalized_words(english_text )
words2 = make_itemsets(english_text2)
english_rules = find_assoc_rules(words2, 0.75)
...
transactions = [set(line.split(',')) for line in groceries_file.split('\n')]
item_counts = defaultdict(int)

for transaction_set in transactions:
    update_item_counts(item_counts, transaction_set)

print("Found {} transactions.".format(len(transactions)))

# Search for an initial set of association rules
initial_rules = find_assoc_rules(transactions, THRESHOLD)

# Filter rules to exclude infrequent items
filtered_rules = {rule: confidence for rule, confidence in initial_rules.items()
                  if item_counts[rule[0]] >= MIN_COUNT}

basket_rules = filtered_rules

basket_rules

#reference: chatgpt for clarification 
