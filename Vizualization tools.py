...
# # Notebook 8 (OPTIONAL): Tools for visualizing data
# 
...
# 
# > If you've encountered GoG ideas before, it was probably when using the best known implementation of GoG, namely, Hadley Wickham's R package, [ggplot2](http://ggplot2.tidyverse.org/).
...
frequencies, edges = np.histogram(flora['petal length'], bins = 5)
...
hv.BoxWhisker(flora['petal length'], label = "Box whiskers plot")
...
p = figure(width=800, height=500, tools=TOOLS)
print(p)
...
p.circle(x=flora['petal width'], y=flora['petal length'])
